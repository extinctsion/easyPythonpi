...
def createarray(length: 'int',
                dtype='int') -> 'array':  # To create an array of entered length and entered data type(interger data type is a default data type)
    import numpy as np
    a = []  # empty list
...
def matrix_mul(matrix1: 'array', matrix2: 'array') -> 'array':
    ...
    if a[1] == b[
        0]:  # from matrix multiplication convention, number of columns of first matrix needs to be equal to number of rows of second matrix
        tt = []
        for i in range(b[0]):
            u = []
            for j in range(b[0]):
...
def convert_number(number, base):
    """
    Converts a number from a given base to the specified base.
    ...
    Raises:
        ValueError: If either 'number' or 'base' is not an integer,
                    'number' is negative, or 'base' is outside the range [2, 36].
    """
    ...
    # Check if 'base' is within the valid range [2, 36]
    if base < 2 or base > 36:
        raise ValueError("The 'base' must be between 2 and 36 inclusive.")

    # Define digits based on the specified base
    ...
    # Return the converted number or "0" if the input number was 0
    return result or "0"
